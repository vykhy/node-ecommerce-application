...
exports.getOrders = async ( req, res ) => {
    ...
    try{
        const orders = await Order.find({ customer: { _id: userId }, deleted: false })
        if(orders.length < 1){
...
exports.deleteOrder = async( req, res ) => {
    ...
    try{
        let order = await Order.findOne({ customer: req.session.uid, _id: orderId })
        if(!order){
            res.send('There was an error')
            return
        }
        if(order.status !== 'completed'){
            success = await Order.findOneAndDelete({ customer: req.session.uid, _id: orderId })
            if(success){
