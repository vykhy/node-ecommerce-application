 ...
 */
exports.filterProductsAndCalculatePrice = async (req, data) => {
  //get items in user's cart
  let dbProducts = await cartService.getCartItems(req);
  //format the data
  ...
  const cartProducts = Object.entries(data).filter(
    (product) => product[1][0] > 0 && product[1][1] == "on"
  );
  ...
  dbProducts.forEach((dbProduct, index) => {
    ...
    dbProduct._doc.sPrice =
      dbProduct._doc.sellingPrice * dbProduct._doc.quantity; //selling price
